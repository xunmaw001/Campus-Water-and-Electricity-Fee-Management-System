<template>
  <div class="main-content">
      ...
      <el-form :inline="true" :model="searchForm" class="form-content">
        <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                  ...
                  <el-input v-if="contents.inputIcon == 0" v-model="searchForm.xingbie" placeholder="性别" clearable></el-input>
                                                                              ...
                                                                              <el-form-item :label="contents.inputTitle == 1 ? '寝室' : ''">
                  <el-input v-if="contents.inputIcon == 1 && contents.inputIconPosition == 1" prefix-icon="el-icon-search" v-model="searchForm.qinshi" placeholder="寝室" clearable></el-input>
                  <el-input v-if="contents.inputIcon == 1 && contents.inputIconPosition == 2" suffix-icon="el-icon-search" v-model="searchForm.qinshi" placeholder="寝室" clearable></el-input>
                  <el-input v-if="contents.inputIcon == 0" v-model="searchForm.qinshi" placeholder="寝室" clearable></el-input>
                </el-form-item>
                                                                                                                <el-form-item>
		    ...
		    label="邮箱">
		     <template slot-scope="scope">
                ...
                <el-button v-if="isAuth('xuesheng','查看') && contents.tableBtnIcon == 0" type="success" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">{{ contents.tableBtnFont == 1?'详情':'' }}</el-button>
                                                                <el-button v-if="isAuth('xuesheng','收费') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1" type="success" icon="el-icon-tickets" size="mini" @click="xueshengjiaofeiCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'收费':'' }}</el-button>
                <el-button v-if="isAuth('xuesheng','收费') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2" type="success" size="mini" @click="xueshengjiaofeiCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'收费':'' }}<i class="el-icon-tickets el-icon--right" /></el-button>
                <el-button v-if="isAuth('xuesheng','收费') && contents.tableBtnIcon == 0" type="success" size="mini" @click="xueshengjiaofeiCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'收费':'' }}</el-button>
    ...
    <add-or-update v-if="addOrUpdateFlag" :parent="this" ref="addOrUpdate"></add-or-update>
            ...
            <xueshengjiaofei-cross-add-or-update v-if="xueshengjiaofeiCrossAddOrUpdateFlag" :parent="this" ref="xueshengjiaofeiCrossaddOrUpdate"></xueshengjiaofei-cross-add-or-update>
...
import xueshengjiaofeiCrossAddOrUpdate from "../xueshengjiaofei/add-or-update";
export default {
  data() {
    return {
                                                                                                                              qinshiOptions: [],
      ...
      addOrUpdateFlag:false,
                  xueshengjiaofeiCrossAddOrUpdateFlag: false,
  ...
  components: {
    AddOrUpdate,
            xueshengjiaofeiCrossAddOrUpdate,
          },
  methods: {
    contentStyleChange() {
    ...
    contentPageStyleChange(){
      ...
      this.contents.pageEachNum = 10
            ...
            xueshengjiaofeiCrossAddOrUpdateHandler(row,type){
      this.showFlag = false;
      this.addOrUpdateFlag = false;
      this.xueshengjiaofeiCrossAddOrUpdateFlag = true;
      this.$storage.set('crossObj',row);
      this.$storage.set('crossTable','xuesheng');
      this.$nextTick(() => {
        this.$refs.xueshengjiaofeiCrossaddOrUpdate.init(row.id,type);
      });
    },
                    init () {
                                                                                this.xingbieOptions = "男,女".split(',')
                                            this.$http({
            url: `option/sushexinxi/sushemingcheng`,
            method: "get"
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.qinshiOptions = data.data;
            } else {
      ...
      let params = {
        ...
        sort: 'id',
            ...
            params['xingbie'] = '%' + this.searchForm.xingbie + '%'
                                                            ...
                                                            if(this.searchForm.qinshi!='' && this.searchForm.qinshi!=undefined){
            params['qinshi'] = '%' + this.searchForm.qinshi + '%'
          }
                                                                                                this.$http({
        url: "xuesheng/page",
        method: "get",
